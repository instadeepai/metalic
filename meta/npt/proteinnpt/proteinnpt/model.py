...
from proteinnpt import utils
from proteinnpt.utils.esm.axial_attention import ColumnSelfAttention, RowSelfAttention
...
class ProteinNPTModel(nn.Module):
    def __init__(self, args, alphabet):
        ...
        self.PNPT_ensemble_test_num_seeds = (
            args.PNPT_ensemble_test_num_seeds
        )  # -1 was hardcoded default previously
        ...
        if self.args.aa_embeddings in ["MSA_Transformer", "ESM1v"]:
            model, _ = utils.esm.pretrained.load_model_and_alphabet(
                args.embedding_model_location
            )
            ...
            self.aa_embedding = (
                utils.tranception.model_pytorch.TranceptionLMHeadModel.from_pretrained(
        ...
        self.target_embedding = nn.ModuleDict(
            {
                ...
                if self.args.target_config[target_name]["type"] == "continuous"
                    ...
                    self.args.target_config[target_name]["dim"], self.args.embed_dim
                )
    ...
    def forward(
        ...
        if self.args.aa_embeddings == "MSA_Transformer" and sequence_embeddings is None:
        ...
        if padding_mask is not None:
            padding_mask_with_targets = torch.zeros(
                num_MSAs_in_batch,
                num_sequences_in_alignments,
        ...
        for target_index, target_name in enumerate(self.target_names):
            if (
                ...
                target_predictions[target_name] = self.target_pred_head[target_name](
                    y
                ).view(
                    -1
    ...
    def forward_with_uncertainty(
        ...
        for (
            ...
            self.modules()
        ):  # Move all dropout layers in train mode to support MC dropout. Keep everything else in eval mode.
            if m.__class__.__name__.startswith("Dropout"):
                m.train()
        with torch.no_grad():
            predictions_dict = defaultdict(list)
    ...
    def protein_npt_loss(
        ...
        if (token_labels is not None) and (MLM_reconstruction_loss_weight > 0.0):
            if (
                ...
                and not self.args.preload_embeddings
            ):
                ...
                if self.args.target_config[target_name]["type"] == "continuous":
                    ...
                    if (
                        ...
                        tgt_loss = MSELoss(reduction="mean")(
                            ...
                            target_labels[target_name][loss_masked_targets],
                        )  # we do not average the loss per batch, so that it's easier to do 1 full average across all batches
                    ...
                    tgt_loss = CrossEntropyLoss(
                        ...
                        target_predictions[target_name].view(
                            -1, self.args.target_config[target_name]["dim"]
                        ),
                        target_labels[target_name].view(-1),
