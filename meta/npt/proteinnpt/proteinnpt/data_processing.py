...
from proteinnpt.utils.data_utils import (
    get_indices_retrieved_embeddings,
    mask_protein_sequences,
    mask_targets,
    slice_sequences,
)
...
def PNPT_sample_training_points_inference(
    training_sequences, sequences_sampling_method, num_sampled_points
):
    replace = True if "with_replacement" in sequences_sampling_method else False
    num_sequences_training_data = len(training_sequences)
    if (num_sequences_training_data <= num_sampled_points) and (not replace):
        return range(num_sequences_training_data)
    ...
    return np.random.choice(
        range(num_sequences_training_data),
        replace=replace,
        p=weights,
        size=num_sampled_points,
    )
...
def process_batch(
    ...
    N.B. training_sequences is a dataset instance.
    """
    target_names = args.target_config.keys()
    target_names_unknown = [
        x for x in args.target_config.keys() if args.target_config[x]["in_NPT_loss"]
    ]
    ...
    for target_name in target_names:
        if target_name not in batch:
            print(
                "Target values were not passed in input batch. We assume all corresponding values are missing & to be predicted."
            )
            batch[target_name] = torch.tensor(
                [np.nan] * number_of_mutated_seqs_to_score
            )  # By construction this will set all labels to -100 in subsequent mask_targets.
            eval_mode = True
        if target_name in target_names_unknown:
            masked_targets, target_labels = mask_targets(
                ...
                if not eval_mode
                else 1.0,  # In eval mode we mask all input targets
                proba_random_mutation=0.1
                if not eval_mode
                else 0.0,  # No random mutation in eval mode
                proba_unchanged=0.1
                if not eval_mode
                else 0.0,  # No unchanged token in eval mode (all masked)
                min_num_labels_masked=1
                if not eval_mode
                else None,  # Masking at least one label during training to have well-defined loss
            )
        else:
            masked_targets, target_labels = mask_targets(
                inputs=batch[target_name],
        ...
        batch_masked_targets[
            target_name
        ] = masked_targets  # masked_targets is of shape (number_of_mutated_seqs_to_score, 2) for continuous targets
        batch_target_labels[
            target_name
        ] = target_labels  # batch_target_labels is of shape (number_of_mutated_seqs_to_score,) for continuous targets

    if args.augmentation == "zero_shot_fitness_predictions_covariate":
        batch_target_labels["zero_shot_fitness_predictions"] = batch[
            "zero_shot_fitness_predictions"
        ].to(
            device
        )  # process fitness pred as a target to make things easier
    if training_sequences is not None:
        ...
        if model.training_sample_sequences_indices is None:
            ...
            num_ensemble_seeds = (
                model.PNPT_ensemble_test_num_seeds
                if model.PNPT_ensemble_test_num_seeds > 0
                else 1
            )
            for ensemble_seed in range(num_ensemble_seeds):
                selected_indices_dict[
                    ensemble_seed
                ] = PNPT_sample_training_points_inference(
        ...
        selected_indices = model.training_sample_sequences_indices[
            selected_indices_seed
        ]
        num_selected_training_sequences = len(
            np.array(training_sequences["mutant_mutated_seq_pairs"])[selected_indices]
        )
        batch["mutant_mutated_seq_pairs"] += list(
            np.array(training_sequences["mutant_mutated_seq_pairs"])[selected_indices]
        )

        # Lightly modified code to concat target sequence and context sequence zero shot values and embeddings if preloaded
        if args.augmentation == "zero_shot_fitness_predictions_covariate":
            if isinstance(training_sequences, datasets.Dataset):
                # some of the type assumptions don't work for dataset which returns list rather than array
                batch_target_labels["zero_shot_fitness_predictions"] = list(
                    batch_target_labels["zero_shot_fitness_predictions"].cpu().numpy()
                )
                batch_target_labels["zero_shot_fitness_predictions"] += list(
                    np.array(training_sequences["zero_shot_fitness_predictions"])[
                        selected_indices
                    ]
                )
                batch_target_labels["zero_shot_fitness_predictions"] = (
                    torch.tensor(batch_target_labels["zero_shot_fitness_predictions"])
                    .float()
                    .to(device)
                )
            else:
                batch_target_labels["zero_shot_fitness_predictions"] = torch.cat(
                    [
                        batch["zero_shot_fitness_predictions"].to(device),
                        torch.tensor(
                            training_sequences["zero_shot_fitness_predictions"][
                                selected_indices
                            ]
                        )
                        .to(batch["zero_shot_fitness_predictions"])
                        .to(device),
                    ]
                )
        # Here's the tricky part: what do we do if embeddings are pre-loaded.
        if "sequence_embeddings" in batch:
            # raise NotImplementedError()  # be very careful about shapes. is there any extra batch dim? NO
            assert batch["sequence_embeddings"].ndim == 3  # num_targets, L, E
            batch["sequence_embeddings"] = torch.cat(
                [
                    batch["sequence_embeddings"],
                    training_sequences["sequence_embeddings"][selected_indices],
                ],
                0,
            )
        for target_name in target_names:
            ...
            masked_training_targets, training_target_labels = mask_targets(
                inputs=torch.tensor(
                    np.array(training_sequences[target_name])[selected_indices]
                ),
                input_target_type=args.target_config[target_name]["type"],
                target_processing=target_processing[target_name],
                proba_target_mask=0.0,
            ...
            batch_masked_targets[target_name] = (
                torch.cat(
                    [batch_masked_targets[target_name], masked_training_targets], dim=0
                )
                .float()
                .to(device)
            )
            num_all_mutated_sequences_input = (
                number_of_mutated_seqs_to_score + num_selected_training_sequences
            )
            assert (
                batch_masked_targets[target_name].shape[0]
                == num_all_mutated_sequences_input
            ), "Error adding training data to seqs to score: {} Vs {}".format(
                batch_masked_targets[target_name].shape[0],
                num_all_mutated_sequences_input,
            )
            batch_target_labels[target_name] = (
                torch.cat([batch_target_labels[target_name], training_target_labels])
                .float()
                .to(device)
            )
            assert (
                batch_masked_targets[target_name].shape[0]
                == batch_target_labels[target_name].shape[0]
            ), "Lengths of masked targets and target labels do not match: {} Vs {}".format(
                batch_masked_targets[target_name].shape[0],
                batch_target_labels[target_name].shape[0],
            )
    else:
        for target_name in target_names:
            batch_masked_targets[target_name] = batch_masked_targets[target_name].to(
                device
            )
            batch_target_labels[target_name] = batch_target_labels[target_name].to(
                device
            )
    ...
    if "sequence_embeddings" in batch:
        ...
        assert torch.is_tensor(sequence_embeddings)
        # TODO: check we have the right number of them...
    else:
        if args.sequence_embeddings_location is not None:
            try:
                indices_retrieved_embeddings = get_indices_retrieved_embeddings(
                    batch, args.sequence_embeddings_location
                )
                assert len(indices_retrieved_embeddings) == len(
                    batch["mutant_mutated_seq_pairs"]
                ), "At least one embedding was missing"
                with h5py.File(args.sequence_embeddings_location, "r") as h5f:
                    sequence_embeddings = torch.tensor(
                        np.array(
                            [h5f["embeddings"][i] for i in indices_retrieved_embeddings]
                        )
                    ).float()
            except:
    ...
    # [I think this can't be ignored: it's necessary for tokens and embeddings to line up]
    ...
    if (
        (args.aa_embeddings == "MSA_Transformer")
        and (MSA_start_position is not None)
        and (MSA_end_position is not None)
        and ((MSA_start_position > 1) or (MSA_end_position < raw_sequence_length))
    ):  # and args.sequence_embeddings_location is None:
        MSA_start_index = MSA_start_position - 1
        MSA_end_index = MSA_end_position
        batch["mutant_mutated_seq_pairs"] = [
            (mutant, seq[MSA_start_index:MSA_end_index])
            for (mutant, seq) in batch["mutant_mutated_seq_pairs"]
        ]
        # Recompute sequence length (has potentially been chopped off above)
        raw_sequence_length = len(batch["mutant_mutated_seq_pairs"][0][1])

    # [This all determines mutant_mutated_seq_pairs. which needs care for loss as well]
    ...
    # We can ignore this since it's skipped.
    if (
        args.aa_embeddings == "MSA_Transformer"
        and sequence_embeddings is None
        and args.num_MSA_sequences_per_training_instance > 0
    ):
        assert (
            MSA_weights is not None
        ), "Trying to add MSA_sequences to scoring batch but no weights are provided"
    ...
    if (
        ...
        if (
            args.long_sequences_slicing_method == "center"
            and args.aa_embeddings == "MSA_Transformer"
        ):
            print(
                "Center slicing method not adapted to MSA Transformer embedding as sequences would not be aligned in the same system anymore. Defaulting to 'left' mode."
            )
    ...
    if (
        args.aa_embeddings == "MSA_Transformer"
        and num_all_mutated_sequences_input > 1
        and sequence_embeddings is None
    ):
        ...
        num_sequences = (
            num_all_mutated_sequences_input
            + args.num_MSA_sequences_per_training_instance
        )
        assert (
            len(batch["mutant_mutated_seq_pairs"]) == num_sequences
        ), "Unexpected number of sequences"
        all_mutated_sequences_input = batch["mutant_mutated_seq_pairs"][
            :num_all_mutated_sequences_input
        ]
        MSA_sequences = batch["mutant_mutated_seq_pairs"][
            num_all_mutated_sequences_input:
        ]
        batch["mutant_mutated_seq_pairs"] = [
            [sequence] + MSA_sequences for sequence in all_mutated_sequences_input
        ]

    if args.aa_embeddings in ["MSA_Transformer", "ESM1v", "Linear_embedding"]:
        if "input_tokens" in batch and training_sequences is None:
            # if training_sequences are passed it's safer to tokenise here
            # TODO: manually concatenate training input_tokens and test input_tokens
            batch_token_sequences = batch["input_tokens"]
        else:
            ...
            ) = token_batch_converter(batch["mutant_mutated_seq_pairs"])
            if (args.aa_embeddings != "MSA_Transformer") or (
                sequence_embeddings is not None
            ):
                (
                    num_MSAs_in_batch,
                    num_sequences_in_alignments,
                    seqlen,
                ) = batch_token_sequences.size()
                batch_token_sequences = batch_token_sequences.view(
                    num_sequences_in_alignments, seqlen
                )  # drop the dummy batch dimension from the tokenizer when using ESM1v / LinearEmbedding
    ...
    batch_masked_tokens, batch_token_labels, masked_indices = mask_protein_sequences(
        ...
        if not eval_mode
        else 0.0,  # In eval mode we do not mask any token
        proba_random_mutation=0.1,
        proba_unchanged=0.1,
    )
