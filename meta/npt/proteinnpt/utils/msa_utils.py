...
def filter_msa(
    ...
    if msa_name == "R1AB_SARS2_02-19-2022_b07":
        hhfilter_max_seq_id = 100  # Otherwise we would only keep 1 sequence.
    ...
    if not os.path.exists(output_filename):
        print("Running hhfilter", output_filename)
        os.system(
            "cat " + filename + ' | tr  "."  "-" >> ' + preprocessed_filename + ".a2m"
        )
        os.system(
            "dd if="
        ...
        print("Output file exists, skipping hhfilter", output_filename)
...
def compute_sequence_weights(MSA_filename, MSA_weights_filename):
    ...
    for seq_name in processed_MSA.raw_seq_name_to_sequence.keys():
        if seq_name == processed_MSA.focus_seq_name:
            MSA_reference_sequence.append(
                (seq_name, processed_MSA.raw_seq_name_to_sequence[seq_name])
            )
            del processed_MSA.seq_name_to_weight[seq_name]
        else:
            if seq_name in processed_MSA.seq_name_to_weight:
                MSA_other_sequences.append(
                    (seq_name, processed_MSA.raw_seq_name_to_sequence[seq_name])
                )
                weights.append(processed_MSA.seq_name_to_weight[seq_name])
    if len(MSA_other_sequences) > 0:
        MSA_non_ref_sequences_weights = (
            np.array(weights)
            / np.array(list(processed_MSA.seq_name_to_weight.values())).sum()
        )
...
def weighted_sample_MSA(
    ...
    msa.extend(
        random.choices(
            MSA_all_sequences[1:],
            weights=MSA_non_ref_sequences_weights,
            k=number_sampled_MSA_sequences - 1,
        )
    )
    msa = [(desc, seq.upper()) for desc, seq in msa]
    return msa
...
def process_MSA(args, MSA_filename, MSA_weights_filename):
    filtered_MSA_filename = filter_msa(
        filename=args.MSA_data_folder + os.sep + MSA_filename,
        path_to_hhfilter=args.path_to_hhfilter,
    )
    print(
        "Filtered MSA filename",
        filtered_MSA_filename,
        os.path.isfile(filtered_MSA_filename),
    )
    MSA_all_sequences, MSA_non_ref_sequences_weights = compute_sequence_weights(
        MSA_filename=filtered_MSA_filename,
        MSA_weights_filename=args.MSA_weight_data_folder
        + os.sep
        + MSA_weights_filename,
    )
    return MSA_all_sequences, MSA_non_ref_sequences_weights


def align_new_sequences_to_msa(
    MSA_sequences, new_sequences, new_mutants, clustalomega_path
):
    """
