...
import tqdm
import wandb
from proteinnpt import baselines, proteinnpt
from scipy.stats import spearmanr

from .data_utils import collate_fn_protein_npt, custom_dict_collator
...
def get_learning_rate(
    ...
    num_total_training_steps=20000,
    ...
    if training_step < num_warmup_steps:
        ...
        ratio_total_steps_post_warmup = (training_step - num_warmup_steps) / (
            num_total_training_steps - num_warmup_steps
        )
...
def get_reconstruction_loss_coefficient(
    training_step,
    num_total_training_steps=20000,
    ...
    reconstruction_loss_coeff = end_MLM_coefficient + cosine_scaler * (
        start_MLM_coefficient - end_MLM_coefficient
    )
...
class Trainer:
    ...
    def train(self):
        ...
        g = torch.Generator()
        g.manual_seed(0)
        print("Num workers", self.args.num_data_loaders_workers)
        # TODO: consider repeating the dataset a certain number of times as an efficiency trick.
        # problem here is that you get repeats within batches.
        ...
        train_loader = torch.utils.data.DataLoader(
            ...
            pin_memory=not self.args.preload_embeddings and not self.args.force_cpu,
            sampler=train_sampler,
            # collate_fn=collate_fn_protein_npt,
            collate_fn=functools.partial(
                custom_dict_collator, list_keys=["mutant_mutated_seq_pairs"]
            ),
        ...
        for training_step in tqdm.tqdm(
            range(1, self.args.num_total_training_steps + 1)
        ):
            optimizer.zero_grad(set_to_none=True)
            lr = scheduler(training_step)
            update_lr_optimizer(optimizer, lr)
            reconstruction_loss_coeff = (
                get_reconstruction_loss_coefficient(
                    training_step,
                    num_total_training_steps=self.args.num_total_training_steps,
                )
                if (
                    self.model.model_type == "ProteinNPT"
                    and not self.model.PNPT_no_reconstruction_error
                )
                else 0
            )
            t0 = time.time()
            for gradient_accum_step in range(self.args.gradient_accumulation):
                try:
                    ...
                    if self.args.verbose:
                        print("ended epoch", num_epochs, flush=True)
                    num_epochs += 1
                    train_iterator = iter(train_loader)
                    batch = next(train_iterator)
                ...
                if self.args.augmentation == "zero_shot_fitness_predictions_covariate":
                    ...
                    del processed_batch["target_labels"][
                        "zero_shot_fitness_predictions"
                    ]
                else:
                ...
                if self.args.training_fp16:
                    ...
                    with torch.cuda.amp.autocast():
                        if self.model.model_type == "ProteinNPT":
                            output = self.model(
                                ...
                                sequence_embeddings=processed_batch[
                                    "sequence_embeddings"
                                ],
                            )
                            (
                            ...
                            ) = self.model.protein_npt_loss(
                                token_predictions_logits=output[
                    ...
                    if self.model.model_type == "ProteinNPT":
                        ...
                        if total_loss.item() > 10.0 and training_step >= 100:
                            print(
                                "High training loss detected: {}".format(
                                    total_loss.item()
                                )
                            )
                    else:
            ...
            for target_name in self.model.target_names:
                log_train_target_prediction_loss_dict[
                    target_name
                ] += target_prediction_loss_dict[target_name]
            if self.model.model_type == "ProteinNPT":
                log_train_reconstruction_loss += reconstruction_loss
                log_train_num_masked_tokens += (
                    processed_batch["masked_tokens"]
                    .eq(self.model.alphabet.mask_idx)
                    .sum()
                )
                for target_name in self.model.target_names:
                    log_train_num_target_masked_tokens_dict[target_name] += (
                        processed_batch["masked_targets"][target_name][:, -1]
                        .eq(1.0)
                        .sum()
                        .item()
                    )  # Masked targets are encoded by 1.0. Mask column is the very last one
            else:
                log_num_sequences_predicted += len(batch["mutant_mutated_seq_pairs"])

            if training_step % self.args.num_logging_training_steps == 0 and (
                self.args.use_wandb or self.args.use_neptune
            ):
                ...
                train_logs = {
                    ...
                    "step_time": delta_time_since_last_log
                    / (self.args.num_logging_training_steps),
                }
                if self.model.model_type == "ProteinNPT":
                    train_logs[
                        "train_total_loss_per_step"
                    ]: log_train_total_loss / self.args.num_logging_training_steps
                    train_logs["train_reconstruction_loss_per_masked_token"] = (
                        log_train_reconstruction_loss / log_train_num_masked_tokens
                    )
                    for target_name in self.model.target_names:
                        train_logs[
                            "train_prediction_"
                            + str(target_name)
                            + "_loss_per_masked_token"
                        ] = (
                            log_train_target_prediction_loss_dict[target_name]
                            / log_train_num_target_masked_tokens_dict[target_name]
                        )
                else:
                    train_logs[
                        "train_total_loss_per_seq"
                    ]: log_train_total_loss / log_num_sequences_predicted
                    for target_name in self.model.target_names:
                        train_logs[
                            "train_prediction_" + str(target_name) + "_loss_per_seq"
                        ] = (
                            log_train_target_prediction_loss_dict[target_name]
                            / log_num_sequences_predicted
                        )
                ...
                if self.args.use_wandb:
                    ...
                    # TODO: check val data here.
                    for key, metric_value in train_logs.items():
                        self.neptune_run[f"npt/train/{key}"].append(
                            metric_value,
                            wait=False,
                        )

            if (
                self.args.save_model_checkpoint
                and (training_step % self.args.num_saving_training_steps) == 0
            ):
                if not os.path.exists(self.args.model_location):
                    os.mkdir(self.args.model_location)
                if not os.path.exists(
                    self.args.model_location
                    + os.sep
                    + "checkpoint-"
                    + str(training_step)
                ):
            ...
            if (
                training_step % self.args.num_eval_steps == 0
                and self.args.use_validation_set
            ):
                if self.model.model_type == "ProteinNPT":
                    ...
                    eval_results = self.eval(
                        test_data=self.val_data, output_all_predictions=True
                    )
                eval_logs = {"Training step": training_step}
                if self.model.model_type == "ProteinNPT":
                    normalization = 0
                    for target_name in self.model.target_names:
                        normalization += eval_results["eval_num_masked_targets"][
                            target_name
                        ]
                else:
                    normalization = eval_results["eval_num_predicted_targets"]
                eval_logs["Eval total loss per seq."]: eval_results[
                    "eval_total_loss"
                ] / normalization
                ...
                for target_name in self.model.target_names:
                    if self.model.model_type == "ProteinNPT":
                        normalization = eval_results["eval_num_masked_targets"][
                            target_name
                        ]  # Update for PNPT (keeep the same normalization constant otherwise)
                    eval_logs["Eval loss " + str(target_name) + " per seq."] = (
                        eval_results["eval_target_prediction_loss_dict"][target_name]
                        / normalization
                    )
                    eval_logs["Eval spearman " + target_name] = spearmanr(
                        eval_results["output_scores"]["predictions_" + target_name],
                        eval_results["output_scores"]["labels_" + target_name],
                    )[0]
                    average_spearman_across_targets += eval_logs[
                        "Eval spearman " + target_name
                    ]
                average_spearman_across_targets /= len(self.model.target_names)
                print(
                    " | ".join(
                        [
                            key + ": " + str(round(eval_logs[key], 5))
                ...
                if self.args.use_neptune:
                    for key, metric_value in eval_logs.items():
                        self.neptune_run[f"npt/val/{key}"].append(
                            metric_value,
                            wait=False,
                        )
                # Early stopping
                all_spearmans_eval_during_training.append(
                    average_spearman_across_targets
                )
                if (
                    average_spearman_across_targets
                    > max_average_spearman_across_targets
                ):
                    max_average_spearman_across_targets = (
                        average_spearman_across_targets
                    )
                if (
                    (training_step >= 1000)
                    and (self.args.early_stopping_patience is not None)
                    and (
                        np.array(all_spearmans_eval_during_training)[
                    ...
                    print(
                        "Early stopping. Training step: {}. Total eval loss: {}. Avg spearman: {}".format(
                            training_step,
                            eval_results["eval_total_loss"],
                            average_spearman_across_targets,
                        )
                    )
                    break
    ...
    def predict(
        ...
        """A custom method. Strips the evaluation code out of eval, so that test data targets can be ignored."""
        self.model.eval()
        self.model.to(self.device)
        self.model.set_device()
        with torch.no_grad():
            eval_loader = torch.utils.data.DataLoader(
                ...
                pin_memory=not self.args.preload_embeddings and not self.args.force_cpu,
                collate_fn=functools.partial(
                    custom_dict_collator,
                    list_keys=["mutant_mutated_seq_pairs"],
            ...
            for batch in tqdm.tqdm(eval_iterator):
                output_scores["mutated_sequence"] += list(
                    zip(*batch["mutant_mutated_seq_pairs"])
                )[1]
                output_scores["mutant"] += list(
                    zip(*batch["mutant_mutated_seq_pairs"])
                )[0]
                if self.model.model_type == "ProteinNPT":
                    processed_batch = proteinnpt.data_processing.process_batch(
                ...
                if self.model.model_type == "ProteinNPT":
                    if mc_dropout_samples is None:
                        ...
                        output = self.model.ensemble_forward(
                            tokens=processed_batch["masked_tokens"],
                            targets=processed_batch["masked_targets"],
                            zero_shot_fitness_predictions=zero_shot_fitness_predictions,
                            sequence_embeddings=processed_batch["sequence_embeddings"],
                ...
                num_of_mutated_seqs_to_score = (
                    processed_batch["num_of_mutated_seqs_to_score"]
                    if self.model.model_type == "ProteinNPT"
                    else len(processed_batch["mutant_mutated_seq_pairs"])
                )
                for target_name in self.model.target_names:
                    # will be a list of arrays in the mc dropout case
                    output_scores["predictions_" + target_name] += list(
                        output["target_predictions"][target_name][
                            :num_of_mutated_seqs_to_score
                        ]
                        .cpu()
                        .numpy()
                    )
                    output_scores["labels_" + target_name] += list(
                        processed_batch["target_labels"][target_name][
                            :num_of_mutated_seqs_to_score
                        ]
                        .cpu()
                        .numpy()
            ...
            # AH: here there's maybe some aggregation required due to e.g. use of MC dropout?
            output_scores_df = pd.DataFrame.from_dict(output_scores)
            output_scores_numeric_cols = [
                col_name
                for col_name in output_scores_df.columns
                if col_name not in ["mutant", "mutated_sequence"]
            ]
            # this is just in case we have multiple inputs for the same mutant
            # we then average the values. when would this happen?
            mutant_counts = output_scores_df.groupby(["mutant"])[
                output_scores_numeric_cols
            ].count()
            assert mutant_counts.max().max() == 1
    ...
    def eval(
        ...
        with torch.no_grad():
            ...
            for batch in tqdm.tqdm(eval_iterator):
                ...
                if self.model.model_type == "ProteinNPT":
                    ...
                    (
                        ...
                        batch_target_prediction_loss_dict,
                    ) = self.model.protein_npt_loss(
                        token_predictions_logits=output["logits_protein_sequence"],
                        token_labels=processed_batch["token_labels"],
                        target_predictions=output["target_predictions"],
                ...
                for target_name in self.model.target_names:
                    eval_target_prediction_loss_dict[
                        target_name
                    ] += batch_target_prediction_loss_dict[target_name].item()
                if self.model.model_type == "ProteinNPT":
                    eval_reconstruction_loss += batch_reconstruction_loss.item()
                    eval_num_masked_tokens += (
                        processed_batch["masked_tokens"]
                        .eq(self.model.alphabet.mask_idx)
                        .sum()
                        .item()
                    ...
                    for target_name in self.model.target_names:
                        eval_num_masked_targets[target_name] += (
                            processed_batch["masked_targets"][target_name][:, -1]
                            .eq(1.0)
                            .sum()
                            .item()
            ...
            output_scores_numeric_cols = [
                col_name
                for col_name in output_scores.columns
                if col_name not in ["mutant", "mutated_sequence"]
            ]
            output_scores = (
                output_scores.groupby(["mutant"])[output_scores_numeric_cols]
                .mean()
                .reset_index()
            )
            mutated_seqs_dict = {}
            ...
            mutated_seqs_dict["mutated_sequence"] = mutant_mutated_seqs[1]
            mutated_seqs_df = pd.DataFrame.from_dict(mutated_seqs_dict)
            output_scores = pd.merge(
                output_scores, mutated_seqs_df, on="mutant", how="left"
            )
        ...
        if need_head_weights:
            print(
                "dimension of first attention column {}".format(col_attentions[0].shape)
            )
            eval_results["col_attentions"] = (
                torch.stack(col_attentions, dim=0).cpu().numpy()
            )
            eval_results["row_attentions"] = (
                torch.stack(row_attentions, dim=0).cpu().numpy()
            )
