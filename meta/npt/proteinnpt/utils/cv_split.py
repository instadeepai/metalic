...
def assign_group_stratified(x):
    ...
    group_numbers = np.repeat(np.arange(0, n_groups), group_size)[
        :n_items
...
def create_folds_random(DMS, n_folds=5, mutant_column="mutant"):
    ...
    mutated_region_list = (
        DMS[mutant_column].apply(lambda x: return_position_single(x)).unique()
    )
...
def create_folds_random_multiples(DMS):
    ...
    DMS["fold_rand_multiples"] = (
        DMS.groupby("mutation_depth")["mutant"]
        .apply(assign_group_stratified)
        .explode()
        .values
    )
    return DMS
...
def create_folds_by_position_modulo(DMS, n_folds=5, mutant_column="mutant"):
    ...
    if len_mutated_region < n_folds:
        raise Exception("Error, there are fewer mutated regions than requested folds")
    DMS[column_name] = (
        DMS[mutant_column].apply(lambda x: return_position_single(x)) % n_folds
    )
    print(DMS[column_name].value_counts())
    return DMS


def create_folds_by_contiguous_position_discontiguous(
    DMS, n_folds=5, mutant_column="mutant", region_mutated=None
):
    ...
    if len_mutated_region < n_folds:
        raise Exception("Error, there are fewer mutated regions than requested folds")
    DMS[column_name] = DMS[mutant_column].apply(
        lambda x: folds_indices[return_position_single(x)]
    )
    print(DMS[column_name].value_counts())
    return DMS


def create_folds_loop(
    reference_file_location=None,
    fold_list=[5],
    dir_path="./ProteinGym/DMS_assays/substitutions",
    saving_folder="./ProteinGym/data/fitness",
):
    ProteinGym_ref = pd.read_csv(reference_file_location, low_memory=False)
    list_errors = []
    for DMS_id, DMS_has_multiples, region_mutated in zip(
        ProteinGym_ref.DMS_id,
        ProteinGym_ref.includes_multiple_mutants,
        ProteinGym_ref.region_mutated,
    ):
        try:
            if DMS_has_multiples:
                ...
                DMS_saving_path = (
                    saving_folder + os.sep + "multiples/" + DMS_id + ".csv"
                )
            ...
            for n_folds in fold_list:
                try:
                    ...
                    DMS = create_folds_by_contiguous_position_discontiguous(
                        DMS, n_folds=n_folds, region_mutated=region_mutated
                    )
                except Exception as e:
                    DMS = create_folds_by_position_modulo(DMS, n_folds=4)
                    DMS = create_folds_random(DMS, n_folds=4)
                    DMS = create_folds_by_contiguous_position_discontiguous(
                        DMS, n_folds=4, region_mutated=region_mutated
                    )
                    DMS = DMS.rename(
                        columns={
                            "fold_modulo_4": "fold_modulo_5",
                            "fold_random_4": "fold_random_5",
                            "fold_contiguous_4": "fold_contiguous_5",
                        }
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="/users/pastin/projects/open_source/ProteinNPT/proteingym/DMS_substitutions.csv",
        type=str,
        help="Path to reference file with list of assays to score",
    )
    parser.add_argument(
        "--input_DMS_files_location",
        default="/scratch/pastin/protein/ProteinNPT/ProteinGym/DMS_assays/substitutions/Tsubo",
        type=str,
        help="Location of ProteinGym DMS files",
    )
    parser.add_argument(
        "--output_DMS_files_location",
        default="/scratch/pastin/protein/ProteinNPT/ProteinGym/DMS_assays/substitutions/Tsubo/cv_folds",
        type=str,
        help="Location of processed DMS files with cross-validation folds included",
    )
