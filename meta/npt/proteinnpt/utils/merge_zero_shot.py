...
def main(args):
    ...
    DMS_data = pd.read_csv(
        args.DMS_mutants_folder + os.sep + DMS_file_name, low_memory=False
    )
    var_list = (
        ["mutant", "mutated_sequence", "DMS_score", "DMS_score_bin"]
        if not args.indel_mode
        else ["mutant", "mutated_sequence", "DMS_score"]
    )
    ...
    model_list = (
        ["Tranception", "ESM1v", "MSA_Transformer", "DeepSequence", "TranceptEVE"]
        if not args.indel_mode
        else ["Tranception"]
    )
    for model_name in model_list:
        print(model_name)
        model_score_file_path = (
            args.zero_shot_scores_folder + os.sep + model_name + os.sep + DMS_file_name
        )
        if os.path.exists(model_score_file_path):
            ...
            if "mutated_sequence" in scores:
                scores = scores[
                    ["mutated_sequence", score_name_mapping_original_names[model_name]]
                ].drop_duplicates()
                merge_key = "mutated_sequence"
            else:
                scores = scores[
                    ["mutant", score_name_mapping_original_names[model_name]]
                ].drop_duplicates()
                merge_key = "mutant"
            scores.columns = [merge_key, score_name_mapping_clean_names[model_name]]
            merge = pd.merge(merge, scores, on=merge_key, how="inner")
            assert num_mutants == len(
                merge
            ), "Some mutants were dropped during the merge: {} (pre-merge) vs {} (post-merge)".format(
                num_mutants, len(merge)
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--DMS_mutants_folder",
        type=str,
        help="Path to folder that contains all mutant files to be scored",
    )
    parser.add_argument(
        "--DMS_index", type=int, help="Index of DMS assay in reference file"
    )
    parser.add_argument(
        "--zero_shot_scores_folder",
        type=str,
        help="Path to folder that contains all zero-shot model scores",
    )
    parser.add_argument(
        "--indel_mode",
        action="store_true",
