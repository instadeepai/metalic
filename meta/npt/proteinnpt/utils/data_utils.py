...
def split_data_based_on_test_fold_index(
    dataframe,
    fold_variable_name="fold_modulo_5",
...
def get_train_val_test_data(args, assay_file_names):
    ...
    assert (
        main_target_name is not None
    ), "No main target referenced. Please update config to select a unique main target."
    assert (
        main_target_name_count <= 1
    ), "Several main targets referenced. Please update config to select a unique main target."

    assay_data[main_target_name] = pd.read_csv(
        args.target_config[main_target_name]["location"]
        + os.sep
        + assay_file_names[main_target_name]
    )[
        [
            "mutant",
            "mutated_sequence",
            args.target_config[main_target_name]["var_name"],
            args.fold_variable_name,
        ]
    ]
    assay_data[main_target_name].columns = [
        "mutant",
        "mutated_sequence",
        main_target_name,
        args.fold_variable_name,
    ...
    for target_name in target_names:
        if target_name != main_target_name:
            ...
            assay_data[target_name] = pd.read_csv(
                args.target_config[target_name]["location"]
                + os.sep
                + assay_file_names[target_name]
            )[["mutant", args.target_config[target_name]["var_name"]]]
    ...
    if args.augmentation == "zero_shot_fitness_predictions_covariate":
        zero_shot_fitness_predictions = pd.read_csv(
            args.zero_shot_fitness_predictions_location
            + os.sep
            + assay_file_names[main_target_name]
        )[["mutant", args.zero_shot_fitness_predictions_var_name]]
        zero_shot_fitness_predictions.columns = [
            "mutant",
            "zero_shot_fitness_predictions",
        ]
        zero_shot_fitness_predictions["zero_shot_fitness_predictions"] = standardize(
    ...
    for split_name, split in zip(["train", "val", "test"], train_val_test_splits):
        if split_name == "val" and not args.use_validation_set:
            continue
        splits_dict[split_name] = {}
        splits_dict[split_name]["mutant_mutated_seq_pairs"] = list(
            zip(list(split["mutant"]), list(split["mutated_sequence"]))
        )
        raw_targets = {target_name: split[target_name] for target_name in target_names}
        if args.augmentation == "zero_shot_fitness_predictions_covariate":
            raw_targets["zero_shot_fitness_predictions"] = split[
                "zero_shot_fitness_predictions"
            ]
        if split_name == "train":
            raw_targets, target_processing = preprocess_training_targets(
                raw_targets, args.target_config
            )
        else:
            raw_targets = preprocess_test_targets(
                raw_targets, args.target_config, target_processing
            )
        for target_name in target_names:
            splits_dict[split_name][target_name] = raw_targets[target_name]
        if args.augmentation == "zero_shot_fitness_predictions_covariate":
            splits_dict[split_name]["zero_shot_fitness_predictions"] = raw_targets[
                "zero_shot_fitness_predictions"
            ]
    # load dict into dataset objects
    train_data = Dataset.from_dict(splits_dict["train"])
    val_data = (
        Dataset.from_dict(splits_dict["val"]) if args.use_validation_set else None
    )
...
def preprocess_training_targets(
    training_targets, target_config
    ...
    for target_name in training_targets.keys():
        if (
            target_name in target_config
            and target_config[target_name]["type"] == "continuous"
        ) or (target_name == "zero_shot_fitness_predictions"):
            # Standard scale
            target_processing[target_name] = {}
            target_processing[target_name]["mean"] = np.nanmean(
                training_targets[target_name]
            )
            target_processing[target_name]["std"] = np.nanstd(
                np.array(training_targets[target_name])
            )
            target_processing[target_name]["P95"] = np.nanquantile(
                np.array(training_targets[target_name]), q=0.95
            )
            print("Target processing train set: {}".format(target_processing))
            training_targets[target_name] = (
                training_targets[target_name] - target_processing[target_name]["mean"]
            ) / target_processing[target_name]["std"]
        else:
            # One-hot encoding
            target_processing[target_name] = {}
            unique_categories = torch.unique(training_targets[target_name])
            assert target_config[target_name]["dim"] == len(
                unique_categories
            ), "list_dim_input_targets not properly referenced for target indexed: {}".format(
                target_name
            )
            ...
            category_to_index["<mask>"] = len(
                unique_categories
            )  # Set <mask> category to largest index value
            index_to_category[len(unique_categories)] = "<mask>"
            training_targets[target_name] = torch.tensor(
                [category_to_index[val] for val in training_targets[target_name]]
            )
    ...
    for target_name in test_targets.keys():
        if (
            ...
            test_targets[target_name] = (
                test_targets[target_name] - target_processing[target_name]["mean"]
            ) / target_processing[target_name]["std"]
        else:
            test_targets[target_name] = torch.tensor(
                [
                    target_processing[target_name]["category_to_index"][val]
                    for val in test_targets[target_name]
                ]
            )
    return test_targets
...
def mask_protein_sequences(
    inputs, alphabet, proba_aa_mask=0.15, proba_random_mutation=0.1, proba_unchanged=0.1
):
    """
    ...
    all_special_tokens = torch.tensor(
        [alphabet.tok_to_idx[x] for x in alphabet.all_special_tokens]
    )
    ...
    labels[
        ...
        -100
    )  # We only compute loss on masked tokens (consequently, all special tokens above will be all excluded from the loss automatically --> including any token that was set to <mask> beforehand (eg., missing values))
    ...
    indices_replaced = (
        torch.bernoulli(torch.full(labels.shape, proba_actual_masking)).bool()
        & masked_indices
    )
    ...
    if proba_random_mutation > 0:
        rescaled_proba = proba_random_mutation / (
            proba_random_mutation + proba_unchanged
        )
        indices_random = (
            torch.bernoulli(torch.full(labels.shape, rescaled_proba)).bool()
            & masked_indices
            & ~indices_replaced
        )
        ...
        )  # we ensure we dont replace by a special token. high in torch.randint is exclusive.
...
def mask_targets(
    ...
    probability_tensor.masked_fill_(
        indices_missing_values, value=0.0
    )  # This will help ensure we do not mask missing values (we have no ground truth values to compute the loss against for these)
    masked_indices = torch.bernoulli(
        probability_tensor
    ).bool()  # tensor indicating which entries should be masked and used in loss computation.

    if (
        min_num_labels_masked is not None and min_num_labels_masked > 0
    ):  # Routine to help enforce we mask at least min_num_labels_masked labels, eg., to ensure we do compute a prediction loss
        if (
            masked_indices.int().sum().item() < min_num_labels_masked
        ):  # We sample additional masks as needed
            num_additional_masks_needed = (
                min_num_labels_masked - masked_indices.int().sum().item()
            )
            non_masked_indices_positions = np.arange(len(masked_indices))[
                np.array(~masked_indices)
            ]  # convert to numpy boolean as torch does not handle well boolean arrays of size 1
            selected_indices = np.random.choice(
                non_masked_indices_positions, size=int(num_additional_masks_needed)
            )
    ...
    labels[
        ...
        -100
    )  # All non masked items (incl. missing values) are excluded from loss computation.
    ...
    if input_target_type == "continuous":
        ...
        inputs[indices_missing_values] = target_processing["category_to_index"][
            "<mask>"
        ]
    # proba_random_mutation of the time (10% by default), we replace masked input tokens with random word
    if proba_random_mutation > 0:
        ...
        if input_target_type == "continuous":
            random_tokens = torch.tensor(
                np.random.normal(loc=0.0, scale=1.0, size=labels.shape)
            ).float()
        else:
            random_tokens = torch.randint(
                low=0,
                high=target_processing["category_to_index"]["<mask>"],
                size=labels.shape,
                dtype=torch.long,
            )
        inputs[indices_random] = random_tokens[indices_random]
    ...
    if input_target_type == "continuous":
        ...
        inputs = torch.cat(
            (inputs.view(-1, 1), indices_masked_or_missing.float().view(-1, 1)), dim=-1
        )
...
def custom_dict_collator(batch, list_keys=None, verbose=False):
    t0 = time.time()
    ...
    if not isinstance(list_keys, list):
        list_keys = [list_keys]
    default_collatable = [
        {k: v for k, v in d.items() if k not in list_keys} for d in batch
    ]
    collated = default_collate(default_collatable)
    collated = {
        k: t.float() if k != "input_tokens" else t.long() for k, t in collated.items()
    }
    collated.update({k: [d[k] for d in batch] for k in list_keys})
    t1 = time.time()
    if verbose:
        print(t1 - t0)
    return collated
...
def collate_fn_protein_npt(raw_batch, verbose=False):
    t0 = time.time()
    keys = raw_batch[0].keys()
    target_keys = list(
        set(keys) - set(["mutant_mutated_seq_pairs"])
    )  # target_keys also includes zero-shot fitness predictions if we pass that as input as well
    batch = {}
    for key in keys:
        batch[key] = []
...
def get_optimal_window(mutation_position_relative, seq_len_wo_special, model_window):
    ...
    if seq_len_wo_special <= model_window:
        ...
        ) < model_window:  # ensures all sequences will be same lengths (catch all for edge cases, due to non integer half_model_window)
            window_end += 1
        return [window_start, window_end]


def slice_sequences(
    list_mutant_mutated_seq_pairs,
    max_positions=1024,
    method="rolling",
    rolling_overlap=100,
    eval_mode=True,
    batch_target_labels=None,
    batch_masked_targets=None,
    target_names=None,
    start_idx=1,
    num_extra_tokens=1,
):
    ...
    if method == "center":
        mutations_barycenters = [
            int(
                np.array(
                    [int(mutation[1:-1]) - start_idx for mutation in mutant.split(":")]
                ).mean()
            )
            for mutant in all_mutants
        ]
        scoring_optimal_window = [
            get_optimal_window(x, raw_sequence_length, max_positions - num_extra_tokens)
            for x in mutations_barycenters
        ]  # Removing 1 from args.max_positions to allow subsequent addition of BOS token
        sliced_mutated_seqs = [
            all_mutated_seqs[index][
                scoring_optimal_window[index][0] : scoring_optimal_window[index][1]
            ]
            for index in range(len(all_mutated_seqs))
        ]
        list_mutant_mutated_seq_pairs = list(zip(all_mutants, sliced_mutated_seqs))
    elif method == "left":
        sliced_mutated_seqs = [
            all_mutated_seqs[index][0 : max_positions - num_extra_tokens]
            for index in range(len(all_mutated_seqs))
        ]  # minus 1 to keep room for BOS token
        list_mutant_mutated_seq_pairs = list(zip(all_mutants, sliced_mutated_seqs))
...
def get_indices_retrieved_embeddings(
    batch, embeddings_dict_location, number_of_mutated_seqs_to_score=None
):
    ...
    mutants_embeddings_df = pd.DataFrame.from_dict(
        mutants_embeddings_dict, orient="columns"
    )
    ...
    intersection = pd.merge(
        batch_mutants_df, mutants_embeddings_df, how="inner", on="mutants"
    )
    return np.array(intersection["mutant_index"].values)


def pnpt_spearmanr(prediction, target):
    mask_missing_values = np.isnan(target) | np.equal(
        target, -100
    )  # In PNPT missing values are never masked so corresponding labels are always set to -100
    return spearmanr(prediction[~mask_missing_values], target[~mask_missing_values])[
        0
    ]  # first value is spearman rho, second is the corresponding p-value
