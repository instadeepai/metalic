...
import pandas as pd
import proteinnpt.utils
import torch
import torch.nn.functional as F
from torch import nn
from torch.nn import CrossEntropyLoss, NLLLoss
from transformers import GPT2PreTrainedModel, PreTrainedTokenizerFast
...
class TranceptionBlockAttention(nn.Module):
    ...
    def _attn(
        self, query, key, value, attention_mask=None, head_mask=None, alibi_bias=None
    ):
                ...
                query_list.append(
                    self.query_depthwiseconv[str(kernel_idx)](
                        query[
                            :,
                            (kernel_idx + 1)
                            * self.num_heads_per_kernel_size : (kernel_idx + 2)
...
class TranceptionModel(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.position_embedding = (
            ...
            if hasattr(config, "position_embedding")
            else "learned"
        )
        if self.position_embedding == "learned":
            ...
            self.slopes = torch.Tensor(
                get_slopes(attn_heads, mode=self.position_embedding)
            )
            # The softmax operation is invariant to translation, and bias functions used are always linear.
            alibi = self.slopes.unsqueeze(1).unsqueeze(1) * torch.arange(
                maxpos
        ...
        self.h = nn.ModuleList(
            [TranceptionBlock(config) for _ in range(config.num_hidden_layers)]
        )
...
class TranceptionLMHeadModel(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if self.retrieval_aggregation_mode is not None:
            ...
            self.MSA_filename = (
                config.MSA_filename if hasattr(config, "MSA_filename") else False
            )
            self.MSA_folder = "/".join(self.MSA_filename.split(os.sep)[:-1])
            self.MSA_name = self.MSA_filename.split(os.sep)[-1]
            self.retrieval_inference_weight_LR = (
                config.retrieval_inference_weight
                if hasattr(config, "retrieval_inference_weight")
                else 0.6
            )
            self.retrieval_inference_weight_RL = (
                config.retrieval_inference_weight
                if hasattr(config, "retrieval_inference_weight")
                else 0.6
            )
            ...
            self.full_protein_length = (
                config.full_protein_length
                if hasattr(config, "full_protein_length")
                else -1
            )
        ...
        if labels is not None:
            ...
            if self.retrieval_aggregation_mode is not None:
                ...
                if self.retrieval_aggregation_mode == "aggregate_indel":
                    assert (
                        batch_size == 1
                    ), "Aggregate indel is only supported for batch size of 1"
                    truncated_sequence_text = mutated_sequence[0][
                        start_slice[0] : end_slice[0]
                    ]
                    if (
                        len(truncated_sequence_text) != shift_logits.shape[1] - 1
                    ):  # shift_logits only has one extra token compared to truncated_sequence_text (the BOS token)
                        print(
                            "Tokenization error -- seq length: {} and shift_logits length - 1 : {}".format(
                    ...
                    (
                        ...
                        MSA_start,
                        MSA_end,
                    ) = msa_utils.update_retrieved_MSA_log_prior_indel(
                        self,
                        self.MSA_log_prior,
                        self.MSA_start,
                        self.MSA_end,
                ...
                for seq_index in range(batch_size):
                    ...
                    if max_prior_slice <= min_prior_slice:
                        print(
                            "Non overlapping region detected: min_prior_slice {} and max_prior_slice {}".format(
                    ...
                    if flip[seq_index]:
                        ...
                        fused_shift_log_probas[
                            seq_index, min_logits_slice:max_logits_slice, :
                        ] = (1 - self.retrieval_inference_weight_RL) * shift_log_probas[
                            seq_index, min_logits_slice:max_logits_slice, :
                        ] + self.retrieval_inference_weight_RL * slice_prior
                    else:
                        ...
                        fused_shift_log_probas[
                            seq_index, min_logits_slice:max_logits_slice, :
                        ] = (1 - self.retrieval_inference_weight_LR) * shift_log_probas[
                            seq_index, min_logits_slice:max_logits_slice, :
                        ] + self.retrieval_inference_weight_LR * slice_prior
                ...
                if self.retrieval_aggregation_mode == "aggregate_indel":
                    try:
                        ...
                        inserted_retrieval_positions = [
                            True if slice_prior[i].sum() == 0 else False
                            for i in range(len(slice_prior))
                        ] + [
                            True
                        ]  # Last True is for the end of sentence token
                        fused_shift_log_probas[
                            :, inserted_retrieval_positions, :
                ...
                loss = loss_fct(
                    input=fused_shift_log_probas.view(
                        -1, fused_shift_log_probas.size(-1)
                    ),
    ...
    def score_mutants(
        ...
        scores_L_to_R = scoring_utils.get_tranception_scores_mutated_sequences(
            model=self,
            mutated_sequence_df=df_left_to_right_slices,
        ...
        if scoring_mirror:
            ...
            df_right_to_left_slices[
                "sliced_mutated_sequence"
            ] = df_right_to_left_slices["sliced_mutated_sequence"].apply(
                lambda x: x[::-1]
            )
            scores_R_to_L = scoring_utils.get_tranception_scores_mutated_sequences(
                model=self,
                mutated_sequence_df=df_right_to_left_slices,
                batch_size_inference=batch_size_inference,
                score_var_name="avg_score_R_to_L",
                target_seq=target_seq,
                num_workers=num_workers,
                reverse=True,
                indel_mode=indel_mode,
            ...
            all_scores["avg_score"] = (
                all_scores["avg_score_L_to_R"] + all_scores["avg_score_R_to_L"]
            ) / 2.0
        else:
        ...
        if target_seq in DMS_data.mutated_sequence.values:
            if scoring_mirror:
                ...
                wt_row = pd.DataFrame(
                    [[target_seq, 0, 0]],
                    columns=["mutated_sequence", "avg_score_L_to_R", "avg_score"],
                )
    ...
    def encode_batch(self, protein_sequence, sequence_name="sliced_mutated_sequence"):
        ...
        protein_sequence[sequence_name] = scoring_utils.sequence_replace(
            sequences=protein_sequence[sequence_name],
            char_to_replace="X",
            char_replacements="ACDEFGHIKLMNPQRSTVWY",
        )
        protein_sequence[sequence_name] = scoring_utils.sequence_replace(
            sequences=protein_sequence[sequence_name],
            char_to_replace="B",
            char_replacements="DN",
        )
        protein_sequence[sequence_name] = scoring_utils.sequence_replace(
            sequences=protein_sequence[sequence_name],
            char_to_replace="J",
            char_replacements="IL",
        )
        protein_sequence[sequence_name] = scoring_utils.sequence_replace(
            sequences=protein_sequence[sequence_name],
            char_to_replace="Z",
            char_replacements="EQ",
        )
        return self.config.tokenizer(
            list(protein_sequence[sequence_name]),
...
def get_tranception_tokenizer():
    ...
    tokenizer = PreTrainedTokenizerFast(
        tokenizer_file=dir_path + os.sep + "utils/tokenizers/Basic_tokenizer",
        unk_token="[UNK]",
        sep_token="[SEP]",
        pad_token="[PAD]",
        cls_token="[CLS]",
