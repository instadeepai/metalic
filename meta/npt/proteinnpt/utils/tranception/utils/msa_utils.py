...
def get_msa_prior(
    ...
    if verbose:
        print(
            "Target seq len is {}, MSA length is {}, start position is {}, end position is {} and vocab size is {}".format(
                len_target_seq, MSA_end - MSA_start, MSA_start, MSA_end, vocab_size
            )
        )

    if filter_MSA:
        if verbose:
            print("Num sequences in MSA pre filtering: {}".format(len(msa_data.keys())))
        ...
        for sequence_name in list_sequence_names:
            ...
            hamming_similarity_seq_ref = np.dot(ref_sequence_hot, seq_hot) / np.dot(
                ref_sequence_hot, ref_sequence_hot
            )
            if hamming_similarity_seq_ref < 0.2:
                del msa_data[sequence_name]
        if verbose:
            print(
                "Num sequences in MSA post filtering: {}".format(len(msa_data.keys()))
            )

    if MSA_weight_file_name is not None:
        if verbose:
            print(
                "Using weights in {} for sequences in MSA.".format(MSA_weight_file_name)
            )
        ...
        if verbose:
            print(
                "Dropped {} sequences from MSA due to absent sequence weights".format(
                    dropped_sequences
                )
            )
    else:
    ...
    if (
        retrieval_aggregation_mode == "aggregate_substitution"
        or retrieval_aggregation_mode == "aggregate_indel"
    ):
        ...
        MSA_weight_norm_counts = np.tile(
            MSA_weight_norm_counts.reshape(-1, 1), (1, vocab_size)
        )
    ...
    if verbose:
        for idx, position in enumerate(msa_prior):
            ...
            if not round(position.sum(), 2) == 1.0:
                print(
                    "Position at index {} does not add up to 1: {}".format(
                        idx, position.sum()
                    )
                )
...
def update_retrieved_MSA_log_prior_indel(
    model, MSA_log_prior, MSA_start, MSA_end, mutated_sequence
):
    """
    ...
    sampled_MSA_location = (
        model.MSA_folder
        + os.sep
        + "Sampled"
        + os.sep
        + "Sampled_"
        + model.MSA_filename.split(os.sep)[-1]
    )

    if not os.path.exists(sampled_MSA_location):
        msa_data = process_msa_data(model.MSA_filename)
        msa_data_sampled = filter_msa(
            msa_data, num_sequences_kept=100000
        )  # If MSA has less than 100k sequences, the sample is identical to original MSA
        with open(sampled_MSA_location, "w") as sampled_write_location:
            for index, key in enumerate(msa_data_sampled):
                ...
                sampled_write_location.write(
                    key_name
                    + "\n"
                    + "\n".join(
                        [
                            msa_data_sampled[key][i : i + 80]
                            for i in range(0, len(msa_data_sampled[key]), 80)
                        ]
                    )
                    + "\n"
                )

    seq_to_align_location = (
        model.MSA_folder
        + os.sep
        + "Sampled"
        + os.sep
    ...
    sequence_text_split = [
        mutated_sequence[i : i + 80] for i in range(0, len(mutated_sequence), 80)
    ]
    sequence_text_split_split_join = "\n".join([">SEQ_TO_SCORE"] + sequence_text_split)
    os.system(
        "echo '" + sequence_text_split_split_join + "' > " + seq_to_align_location
    )

    expanded_MSA_location = (
        model.MSA_folder
        + os.sep
        + "Sampled"
        + os.sep
    ...
    clustalw_cline = ClustalOmegaCommandline(
        cmd=model.config.clustal_omega_location,
    ...
    aligned_seqA, aligned_seqB = (
        msa_data[">SEQ_TO_SCORE"],
        msa_data[">REFERENCE_SEQUENCE"],
    )
    try:
        keep_column = []
        for column_index_pairwise_alignment in range(len(aligned_seqA)):
            if (
                aligned_seqA[column_index_pairwise_alignment] == "-"
                and aligned_seqB[column_index_pairwise_alignment] == "-"
            ):
                continue
                ...
                MSA_log_prior = torch.cat(
                    (
                        MSA_log_prior[:column_index_pairwise_alignment],
                        torch.zeros(MSA_log_prior.shape[1]).view(1, -1).cuda(),
                        MSA_log_prior[column_index_pairwise_alignment:],
                    ),
                    dim=0,
                )
        ...
        print(
            "Error when processing the following alignment: {}".format(
                expanded_MSA_location
            )
        )
                ...
                print(
                    ...
                    + str(
                        round(
                            float(
                                1
                                - seq_below_threshold.sum() / seq_below_threshold.shape
                            )
                            * 100,
                            2,
                        )
