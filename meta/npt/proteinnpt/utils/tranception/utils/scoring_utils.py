...
def sequence_replace_single(sequence, char_to_replace, char_replacements):
    ...
    replacements = np.random.choice(
        a=char_replacements, size=len(positions), replace=True
    )
...
def get_tranception_scores_mutated_sequences(
    model,
    mutated_sequence_df,
    ...
    with torch.no_grad():
        ...
        for encoded_batch in tqdm.tqdm(ds_loader):
            ...
            if (hasattr(model.config, "retrieval_aggregation_mode")) and (
                ...
                encoded_batch[
                    "mutated_sequence"
                ] = mutated_sequence  # only mutated_sequence is flipped if the scoring_mirror branch of score_mutants. No need to flip mutated_sequence for MSA re-aligning
                fused_shift_log_probas = model(
                    **encoded_batch, return_dict=True
    ...
    if model.config.scoring_window == "sliding":
        scores = (
            scores[["mutated_sequence", "score"]]
            .groupby("mutated_sequence")
            .sum()
            .reset_index()
    ...
    if target_seq is not None:
        ...
        if model.config.scoring_window == "optimal":
            ...
            delta_scores[score_var_name] = (
                delta_scores["score"] - delta_scores["score_wt"]
            )
        elif model.config.scoring_window == "sliding":
            delta_scores = scores_mutated_seq.copy()
            delta_scores[score_var_name] = (
                delta_scores["score"] - list(scores_wt["score"])[0]
...
def get_sequence_slices(
    ...
    if scoring_window == "optimal":
        df["mutation_barycenter"] = (
            ...
            if not indel_mode
            else df["mutated_sequence"].apply(lambda x: len(x) // 2)
        )
        df["scoring_optimal_window"] = (
            df["mutation_barycenter"].apply(
                lambda x: get_optimal_window(x, len_target_seq, model_context_len)
            )
            if not indel_mode
            else df["mutated_sequence"].apply(lambda x: (0, len(x)))
        )
        df["sliced_mutated_sequence"] = [
            df["mutated_sequence"][index][
                df["scoring_optimal_window"][index][0] : df["scoring_optimal_window"][
                    index
                ][1]
            ]
        ...
        if (
            indel_mode
        ):  # For indels, we set the wild type reference to be always the same (full length) sequence. We assume here that the length is lower than model context size (otherwise "Sliding" mode should be used)
        ...
        for window_index in range(1, num_windows + 1):
            df_sliced = df.copy()
            df_sliced["sliced_mutated_sequence"] = df_sliced["mutated_sequence"].map(
            ...
            df_sliced_wt["sliced_mutated_sequence"] = df_sliced_wt[
                "mutated_sequence"
            ].map(lambda x: x[start : start + model_context_len])
